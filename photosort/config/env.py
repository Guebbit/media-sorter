...
3. the runtime overlay the web UI and `photosort config set` write, for the
...
def _build_paths(layers: _Layers, overlay_path: Path, models_dir: Path) -> Paths:
    ...
    return Paths(
        database=Path(layers.str_("DB", "data/photosort.db")).expanduser(),
...
def load_settings(env_file: Path | str | None = None) -> Settings:
    ...
    `env_file` says where the `.env` is; `PHOTOSORT_ENV_FILE` does the same from
