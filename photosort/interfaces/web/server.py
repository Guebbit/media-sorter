...
class _Handler(BaseHTTPRequestHandler):
    ...
    def _dispatch(self, build_routes: Callable[[], dict[str, Callable[[], None]]], path: str,
                 catch: tuple[type[Exception], ...] = (PhotosortError,)) -> None:
    ...
    def do_POST(self) -> None:  # noqa: N802
        ...
        self._dispatch(routes, parsed.path, catch=(PhotosortError, KeyError, ValueError))
...
def serve(ctx: AppContext) -> None:
    ...
    log.info("photosort web UI on http://%s:%s", host, port)
    try:
