...
def test_only_the_borderline_classes_are_worth_asking_about():
    assert uncertain_classes([MAYBE_CAT], CONF, REVIEW) == ["cat"]
    assert uncertain_classes([SURE_CAT], CONF, REVIEW) == []
    assert uncertain_classes([], CONF, REVIEW) == []


def test_a_class_seen_confidently_elsewhere_in_the_frame_is_not_in_doubt():
    """One sure cat and one blurry one is not an open question about cats."""
    assert uncertain_classes([SURE_CAT, MAYBE_CAT], CONF, REVIEW) == []


def test_a_detection_below_the_review_band_is_not_escalated():
    """Below the review threshold it was never stored as doubt, only as noise."""
    assert uncertain_classes([Detection("cat", 0.2, 0, 0, 1, 1)], CONF, REVIEW) == []
...
def test_the_escalation_queue_is_exactly_what_raises_the_review_flag(cats):
    ...
    for detections in ([MAYBE_CAT], [SURE_CAT], [], [FAINT_CAT]):
        flagged = decide(detections, cats, CONF, REVIEW).needs_review
        assert flagged == bool(uncertain_classes(detections, CONF, REVIEW))
...
def test_present_promotes_the_photo_to_a_real_match(cats):
    result = adjudicated([MAYBE_CAT], [Adjudication("cat", PRESENT, 0.9)], CONF)
    assert decide(result, cats, CONF, REVIEW).category == "cat"
    assert decide(result, cats, CONF, REVIEW).needs_review is False


def test_absent_drops_the_class_and_the_doubt_with_it(cats):
    result = adjudicated([MAYBE_CAT], [Adjudication("cat", ABSENT, 0.9)], CONF)
    assert result == ()
    decision = decide(result, cats, CONF, REVIEW)
    assert decision.category == "none"
    assert decision.needs_review is False


def test_unsure_changes_nothing_at_all(cats):
    result = adjudicated([MAYBE_CAT], [Adjudication("cat", UNSURE, 0.1)], CONF)
    assert result == (MAYBE_CAT,)
    assert decide(result, cats, CONF, REVIEW).needs_review is True
...
def test_a_verdict_never_overrules_a_confident_detection(cats):
    ...
    result = adjudicated([SURE_CAT], [Adjudication("cat", ABSENT, 1.0)], CONF)
    assert result == (SURE_CAT,)


def test_presence_is_not_a_count():
    ...
    result = adjudicated(
        [MAYBE_CAT, FAINT_CAT], [Adjudication("cat", PRESENT, 0.9)], CONF
    )
    assert [d.confidence for d in result] == [0.9, 0.4]


def test_a_promoted_detection_carries_the_verdicts_own_confidence():
    ...
    (promoted,) = adjudicated([MAYBE_CAT], [Adjudication("cat", PRESENT, 0.95)], CONF)
    assert promoted.confidence == 0.95
    (floored,) = adjudicated([MAYBE_CAT], [Adjudication("cat", PRESENT, 0.1)], CONF)
    assert floored.confidence == CONF


def test_classes_nobody_was_asked_about_are_left_alone():
    result = adjudicated([MAYBE_CAT, MAYBE_DOG], [Adjudication("cat", ABSENT, 0.9)], CONF)
    assert result == (MAYBE_DOG,)


def test_no_verdicts_is_the_identity():
    assert adjudicated([MAYBE_CAT], [], CONF) == (MAYBE_CAT,)
